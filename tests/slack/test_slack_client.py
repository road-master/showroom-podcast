...
class TestSlackClient:
    ...
    @staticmethod
    def test_post_message(mock_slack_web_client_broken_process_pool: MockSlackWebClient):
        """
    ...
    def test_post_message_slack_api_error(mock_slack_web_client_raise_slack_api_error: MockSlackWebClient):
        slack_channel = "slack_channel"
        message = "BrokenProcessPool: A child process terminated abruptly, the process pool is not usable anymore"
    ...
    @staticmethod
    def test_post_reply(mock_slack_web_client_broken_process_pool: MockSlackWebClient):
        """
        ...
        message = "BrokenProcessPool: A child process terminated abruptly, the process pool is not usable anymore"
        actual = SlackClient(MockSlackWebClient.BOT_TOKEN_FOR_TEST, slack_channel).post_reply(message, thread_timestamp)
        assert actual == mock_slack_web_client_broken_process_pool.responses[0]
        assert html.unescape(actual["message"]["text"]) == message, actual["message"]["text"] + message
        mock_slack_web_client_broken_process_pool.constructor.assert_called_once_with(
            token=MockSlackWebClient.BOT_TOKEN_FOR_TEST
        ...
        mock_slack_web_client_broken_process_pool.chat_post_message.assert_called_once_with(
            channel=slack_channel, text=message, thread_ts=thread_timestamp
        )

    @staticmethod
    # Reason: pytest fixture. pylint: disable=unused-argument
    def test_post_reply_slack_api_error(mock_slack_web_client_raise_slack_api_error: MockSlackWebClient):
    ...
    def test_post_error(self, mock_slack_web_client_type_error: MockSlackWebClient):
        """
    ...
    @staticmethod
    def execute_test(slack_bot_token, slack_channel):
        try:
            "2" + 2
